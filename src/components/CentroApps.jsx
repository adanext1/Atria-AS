...
import { inyeccionesPorApp } from '../utils/inyecciones-apps';
...
export default function CentroApps({ volverAlDashboard }) {
  ...
  const [appEfimera, setAppEfimera] = useState(null);
  const [modalAyuda, setModalAyuda] = useState(false);
  const [boostEfimero, setBoostEfimero] = useState(false);
  ...
  const iconInvoice = <svg viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zm-1 1.5L18.5 9H13V3.5zM6 20V4h5v6h6v10H6zm2-3h8v2H8v-2zm0-4h8v2H8v-2zm0-4h5v2H8V9z" /></svg>;
  ...
  const catalogoApps = [
    ...
    // Facturación
    { idBase: 'walmart_fact', nombre: 'Walmart Facturación', desc: 'Portal oficial para descargar tus facturas y XML de compras de Walmart.', categoria: 'Facturación', url: 'https://facturacion.walmartmexico.com.mx', iconoElemento: iconInvoice, color: 'text-blue-500 hover:text-blue-600' },
    { idBase: 'soriana_fact', nombre: 'Soriana Facturación', desc: 'Genera o recupera facturas electrónicas de tiendas Soriana.', categoria: 'Facturación', url: 'https://www.soriana.com/facturacion-electronica', iconoElemento: iconInvoice, color: 'text-orange-500 hover:text-orange-600' },
    { idBase: 'costco_fact', nombre: 'Costco Facturación', desc: 'Tramita facturas a partir de tickets emitidos en Costco.', categoria: 'Facturación', url: 'https://wwwfactura.costco.com.mx/login', iconoElemento: iconInvoice, color: 'text-red-600 hover:text-red-700' },
    { idBase: 'little_cfact', nombre: 'Little Caesars Facturación', desc: 'Facturación rápida de tus pedidos en Little Caesars Pizza.', categoria: 'Facturación', url: 'https://www.infofactura.com/LittleCaesars', iconoElemento: iconInvoice, color: 'text-yellow-600 hover:text-yellow-700' },
  ];

  const categorias = ['Todas', 'Comunicación', 'Productividad', 'Entretenimiento', 'Facturación'];
  ...
  return (
    <div className={`min-h-screen font-sans flex flex-col relative transition-all duration-500 w-full bg-transparent pt-16 md:pt-20 ${appEfimera ? 'max-w-none p-2 md:p-4' : 'max-w-7xl mx-auto p-4 md:p-8'}`}>
      ...
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none z-0 overflow-hidden fixed bg-gray-50 dark:bg-[#0f141e]">
        ...
        <div className="absolute bottom-1/4 left-1/4 w-[35rem] h-[35rem] bg-pink-400/20 dark:bg-pink-600/20 rounded-full mix-blend-multiply dark:mix-blend-screen filter blur-3xl opacity-60 animate-float-reverse delay-1000"></div>
      ...
      {appEfimera ? (
        // VISTA EFÍMERA
        <div className="flex-1 flex flex-col h-full animate-fade-in gap-4 w-full relative z-10">
          {/* HEADER TIPO DASHBOARD PARA APP EFÍMERA */}
          <header className="flex justify-between items-center bg-white/40 dark:bg-slate-800/40 backdrop-blur-md p-4 rounded-2xl shadow-sm border border-white/50 dark:border-slate-700/50 transition-all shrink-0">
            <div className="flex items-center gap-4">
              <button
                onClick={() => { setAppEfimera(null); setBoostEfimero(false); }}
                className="p-2 bg-gray-50 dark:bg-slate-700 text-gray-500 dark:text-gray-400 hover:text-pink-600 dark:hover:text-pink-400 hover:bg-pink-50 dark:hover:bg-pink-900/30 rounded-xl transition-all"
                title="Volver al Centro de Apps"
              >
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white bg-slate-800 shadow-inner ${appEfimera.color.split(' ')[0]}`}>
                  {appEfimera.iconoElemento}
                </div>
                <div>
                  <h1 className="text-xl font-black text-gray-800 dark:text-white flex items-center gap-2">
                    {appEfimera.nombre}
                    <span className="text-[10px] bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 px-2 py-0.5 rounded-md uppercase tracking-wider font-black ml-2">Sesión Temporal</span>
                  </h1>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {inyeccionesPorApp[appEfimera.idBase] && (
                <button
                  onClick={() => {
                    const wv = document.getElementById('visor-temporal-webview');
                    if (wv) {
                      if (!boostEfimero) {
                        const scriptPayload = inyeccionesPorApp[appEfimera.idBase].encender;
                        wv.executeJavaScript(typeof scriptPayload === 'function' ? scriptPayload('[]') : scriptPayload)
                          .then(() => setBoostEfimero(true))
                          .catch(e => alert("Error al inyectar script: " + e.message));
                      } else {
                        wv.executeJavaScript(inyeccionesPorApp[appEfimera.idBase].apagar)
                          .then(() => setBoostEfimero(false));
                      }
                    }
                  }}
                  className={`flex items-center gap-2 px-4 py-2 flex-col md:flex-row rounded-xl font-bold text-sm transition-colors border ${boostEfimero ? 'bg-pink-500 text-white border-pink-500 shadow-md shadow-pink-500/20' : 'bg-pink-50 text-pink-600 hover:bg-pink-100 dark:bg-pink-900/30 dark:hover:bg-pink-900/50 dark:text-pink-400 border-pink-200 dark:border-pink-800'}`}
                >
                  <svg className={`w-5 h-5 ${boostEfimero ? 'animate-bounce' : ''}`} fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.5a.75.75 0 01.743.648l.007.102v3.013a5.5 5.5 0 014.288 3.518l.847 2.222a1.751 1.751 0 00.785.83l.115.056c1.116.495 1.583 1.838.995 2.868l-.089.138A7.834 7.834 0 0113.5 19v2.75a.75.75 0 01-1.493.102L12 21.75V19a7.834 7.834 0 01-6.19-3.048 2.25 2.25 0 01.906-3.006l.115-.056a1.75 1.75 0 00.785-.83l.847-2.222a5.5 5.5 0 014.288-3.518V3.25A.75.75 0 0112 2.5z" /></svg>
                  <span className="hidden md:inline">{boostEfimero ? 'Modo Boost Activo' : 'Activar Boost'}</span>
                </button>
              )}
            </div>
          </header>

          {/* CONTENEDOR FLEX-1 WEBVIEW */}
          <div className="flex-1 bg-white/60 dark:bg-slate-900/40 backdrop-blur-sm rounded-2xl shadow-sm border border-white/40 dark:border-slate-800/50 overflow-hidden relative min-h-[70vh]">
            <webview
              id="visor-temporal-webview"
              src={appEfimera.url}
              partition={`persist:temp-${appEfimera.idBase}`}
              allowpopups="true"
        ...
        <div className="flex flex-col h-full animate-fade-in w-full relative z-10">
          ...
          <header className="flex justify-between items-center mb-8 bg-white/40 dark:bg-slate-800/40 backdrop-blur-md p-4 rounded-2xl shadow-sm border border-white/50 dark:border-slate-700/50 transition-all shrink-0">
            ...
            <button onClick={() => setModalAyuda(true)} className="flex md:items-center flex-col md:flex-row gap-2 px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:text-blue-400 rounded-xl font-bold text-sm transition-colors border border-blue-200 dark:border-blue-800">
              <svg className="w-5 h-5 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
              <span className="hidden md:inline">Descubre Atria Boost</span>
            </button>
          </header>
          ...
          <div className="flex gap-2 mb-6 overflow-x-auto pb-2 custom-scrollbar shrink-0">
            {categorias.map(cat => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-bold whitespace-nowrap transition-all border ${filtro === cat
                  ? 'bg-indigo-600/90 dark:bg-indigo-500/90 text-white border-indigo-400/50 shadow-lg shadow-indigo-500/20 transform -translate-y-0.5'
                  : 'bg-white/40 dark:bg-slate-800/40 text-gray-500 dark:text-slate-400 hover:bg-white/60 dark:hover:bg-slate-700/60 border-white/50 dark:border-slate-700/50 hover:shadow-sm'
                  }`}
              >
                {cat}
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-8">
            {appsFiltradas.map(app => {
              ...
              return (
                <div key={app.idBase} className="bg-white/40 dark:bg-slate-800/40 backdrop-blur-md rounded-3xl p-6 shadow-sm border border-white/50 dark:border-slate-700/50 hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col h-full group relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-white/20 to-transparent dark:from-white/5 dark:to-transparent opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none"></div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      ...
                      {inyeccionesPorApp[app.idBase] && (
                        <span className="text-[10px] font-bold text-pink-600 bg-pink-50 dark:bg-pink-900/30 dark:text-pink-400 px-2 py-0.5 rounded-md flex items-center gap-1 border border-pink-100 dark:border-pink-800/50">
                          ⚡ Soporta Boost
                        </span>
                      )}
                    </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-100 dark:border-slate-700 flex flex-col gap-2">
                    {numInstancias > 0 ? (
                      <div className="flex items-center justify-between bg-emerald-50 dark:bg-emerald-900/10 p-2 rounded-xl border border-emerald-100 dark:border-emerald-800/30 mb-1">
                        <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400 px-2 flex items-center gap-1.5">
                          <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                          {numInstancias} Instancias activas
                        </span>
                        <button
                          onClick={() => abrirModalInstalacion(app)}
                          className="text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 dark:hover:text-emerald-300 transition-colors p-1.5 rounded-lg hover:bg-emerald-100 dark:hover:bg-emerald-800/50"
                          title="Instalar módulo adicional"
                        >
                      ...
                      >
                        ...
                        Conectar Permanemente
                      </button>
                    )}

                    <button
                      onClick={() => setAppEfimera(app)}
                      className="w-full py-2 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl font-bold text-sm transition-colors flex justify-center items-center gap-2"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                      Abrir Ahí Mismo (Temporal)
                    </button>
                  </div>
                </div>
              );
      ...
      {modal.abierto && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-gray-900/20 dark:bg-black/40 backdrop-blur-md" onClick={() => setModal({ abierto: false, appBase: null, nombrePersonalizado: '' })}></div>

          <div className="bg-white/80 dark:bg-slate-800/90 backdrop-blur-xl rounded-3xl shadow-2xl w-full max-w-md p-6 relative z-10 animate-fade-in-up border border-white/50 dark:border-slate-700/50">
      ...
      {/* ================================================================= */}
      {/* 5. MODAL DE AYUDA EDUCATIVO (ONBOARDING) */}
      {/* ================================================================= */}
      {modalAyuda && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 lg:p-10">
          <div className="absolute inset-0 bg-gray-900/10 dark:bg-black/30 backdrop-blur-xl" onClick={() => setModalAyuda(false)}></div>

          <div className="bg-white/60 dark:bg-slate-900/40 backdrop-blur-2xl rounded-[2rem] shadow-2xl w-full max-w-5xl h-full max-h-[85vh] flex flex-col overflow-hidden relative z-10 animate-fade-in-up border border-white/40 dark:border-slate-800/50">
            {/* Header del Modal */}
            <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-gray-50/50 dark:bg-slate-900/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-black text-gray-800 dark:text-white">Descubre Atria Boost</h2>
                  <p className="text-sm font-medium text-gray-500 dark:text-slate-400">La magia técnica detrás del Centro de Apps de tu ERP.</p>
                </div>
              </div>
              <button onClick={() => setModalAyuda(false)} className="bg-gray-100 hover:bg-gray-200 text-gray-500 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-gray-400 p-2.5 rounded-full transition-colors">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
              </button>
            </div>

            {/* Contenido / Bento Grid Educativo */}
            <div className="p-8 overflow-y-auto flex-1 bg-gray-50 dark:bg-slate-900/80">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">

                {/* Bento Card 1: Aislamiento */}
                <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-slate-700">
                  ...
                  <h3 className="text-xl font-black text-gray-800 dark:text-white mb-3">Multicuentas e Instancias</h3>
                  <p className="text-gray-500 dark:text-slate-400 leading-relaxed font-medium">
                    En navegadores normales (Chrome o Edge), si abres WhatsApp Web, solo puedes tener **una sola cuenta activa**.
                    Atria usa particiones nativas para aislar cada app como si fuera "una computadora distinta". Puedes conectar múltiples números de WhatsApp Empresa o varios correos de Gmail simultáneamente sin que cierren tu otra sesión.
                  </p>
                </div>

                {/* Bento Card 2: Modo Boost */}
                <div className="bg-gradient-to-br from-gray-900 to-black rounded-3xl p-8 shadow-xl text-white relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-pink-500/20 rounded-full blur-3xl transform translate-x-1/2 -translate-y-1/2"></div>
                  <div className="w-14 h-14 bg-pink-500/20 backdrop-blur-sm rounded-2xl flex items-center justify-center mb-6 relative z-10 border border-pink-500/30">
                    <svg className="w-7 h-7 text-pink-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.5a.75.75 0 01.743.648l.007.102v3.013a5.5 5.5 0 014.288 3.518l.847 2.222a1.751 1.751 0 00.785.83l.115.056c1.116.495 1.583 1.838.995 2.868l-.089.138A7.834 7.834 0 0113.5 19v2.75a.75.75 0 01-1.493.102L12 21.75V19a7.834 7.834 0 01-6.19-3.048 2.25 2.25 0 01.906-3.006l.115-.056a1.75 1.75 0 00.785-.83l.847-2.222a5.5 5.5 0 014.288-3.518V3.25A.75.75 0 0112 2.5z" /></svg>
                  </div>
                  <h3 className="text-xl font-black mb-3 relative z-10">La magia del "Modo Boost" ⚡</h3>
                  <p className="text-gray-300 leading-relaxed font-medium relative z-10">
                    Bajo el capó, las aplicaciones marcadas con la pestaña ⚡ le permiten al ERP inyectar JavaScript puro en tiempo real para **mutar y mejorar** el código de la web que visitas. Con el Boost activo el ERP lee tu Bóveda, cruza clientes y rellena tus facturas mágicamente modificando el DOM (la estructura) del navegador.
                  </p>
                </div>

                {/* Bento Card 3: Efímeras */}
                <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-slate-700 lg:col-span-2 flex flex-col md:flex-row items-center gap-8">
                  <div className="w-20 h-20 bg-emerald-50 dark:bg-emerald-900/30 rounded-3xl flex items-center justify-center shrink-0 border border-emerald-100 dark:border-emerald-800/30">
                    <svg className="w-10 h-10 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                  </div>
                  <div>
                    <h3 className="text-xl font-black text-gray-800 dark:text-white mb-3">Sesiones Efímeras (On-Demand)</h3>
                    <p className="text-gray-500 dark:text-slate-400 leading-relaxed font-medium">
                      ¿Necesitas descargar una simple factura pero no quieres saturar tu panel lateral permanentemente? Usa el botón <span className="text-blue-500 font-bold">Abrir Ahí Mismo</span>. Atria creará un túnel gráfico temporal en pantalla completa. Podrás aprovechar tus cuentas guardadas y el Modo Boost, y al cerrar con la X, desaparecerá de tu espacio de trabajo.
                    </p>
                  </div>
                </div>
            ...
            {/* Footer Modal */}
            <div className="p-6 bg-white dark:bg-slate-900 border-t border-gray-100 dark:border-slate-800 flex justify-end">
              <button onClick={() => setModalAyuda(false)} className="px-8 py-3 bg-gray-900 dark:bg-white text-white dark:text-gray-900 font-bold rounded-xl hover:scale-105 transition-transform">
                ¡Entendido, a volar! 🚀
              </button>
            </div>
          </div>
