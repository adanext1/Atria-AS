...
const VentanaFlotante = ({ app, icono, abierta, encendida, silenciada, reloadTrigger, onClose, traerAlFrente, zIndex }) => {
  ...
  useEffect(() => {
    ...
    const actualizarNavegacion = () => setPuedeRetroceder(wv.canGoBack());
    ...
    >
      <div onMouseDown={iniciarArrastre} className="bg-gray-100 dark:bg-slate-900 px-3 py-2 flex justify-between items-center select-none cursor-move border-b border-gray-200 dark:border-slate-800">
        <div className="flex items-center gap-2">
          <button onMouseDown={(e) => e.stopPropagation()} onClick={retroceder} disabled={!puedeRetroceder} className={`p-1.5 rounded-lg transition-all ${puedeRetroceder ? 'text-gray-600 hover:text-gray-900 hover:bg-gray-200 dark:text-gray-400 dark:hover:text-white dark:hover:bg-slate-700 cursor-pointer' : 'text-gray-300 dark:text-slate-700 cursor-not-allowed'}`} title="Volver atrás">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"></path></svg>
          </button>
          <div className="w-4 h-4 text-gray-500 dark:text-gray-400 pointer-events-none">{icono}</div> 
          <span className="font-bold text-sm text-gray-700 dark:text-gray-300 pointer-events-none">{app.nombre}</span>
          {silenciada && <span className="text-red-500 ml-1" title="App Silenciada">🔇</span>}
        </div>
        <button onMouseDown={(e) => e.stopPropagation()} onClick={onClose} className="text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 p-1.5 rounded-lg transition-colors cursor-pointer" title="Minimizar">
      ...
      <div className="flex-1 relative bg-[#f0f2f5] dark:bg-[#111b21]">
        ...
        <webview ref={webviewRef} src={app.url} partition={app.particion} allowpopups="true" useragent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" className="absolute inset-0 w-full h-full border-none z-10"></webview>
