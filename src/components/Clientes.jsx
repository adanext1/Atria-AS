...
export default function Clientes({ alVolver, modoOscuro, toggleTema, irANuevoCliente, irAEditarCliente, irADetalleCliente }) {
    ...
    return (
        <div className="min-h-screen p-4 md:p-8 font-sans max-w-7xl mx-auto pb-20">
            ...
            <header className="animate-fade-in-up flex justify-between items-center mb-6 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700">
                <button
                    onClick={alVolver}
                    className="group flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:text-cyan-600 dark:hover:text-cyan-400 font-semibold transition"
                >
                    <div className="bg-gray-100 dark:bg-slate-700 group-hover:bg-cyan-100 dark:group-hover:bg-cyan-900/40 p-1.5 rounded-lg transition-colors">
            ...
            <div className="animate-fade-in-up delay-100 mb-8 bg-gradient-to-br from-white to-gray-50 dark:from-slate-800 dark:to-slate-800/80 rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 dark:border-slate-700 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-cyan-500/5 dark:bg-cyan-400/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
                    <div>
                        <h2 className="text-3xl font-black text-gray-800 dark:text-white flex items-center gap-3 tracking-tight">Directorio de Clientes</h2>
                        <p className="text-gray-500 dark:text-slate-400 mt-2 text-sm md:text-base max-w-xl">
                            Aquí se encuentran tus clientes a quienes les facturas. Lleva el control de ventas y cobranza.
                        </p>
                    </div>
                    {!cargando && clientes.length > 0 && (
                        <div className="flex gap-4 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
                            <Tooltip texto="Suma histórica de todas las ventas cobradas.">
                                <div className="bg-white dark:bg-slate-900 px-4 py-3 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm min-w-[140px] flex-shrink-0 cursor-help">
                                    <p className="text-[10px] uppercase font-bold text-gray-400 dark:text-slate-500 mb-1">Total Ventas</p>
                                    <p className="text-lg font-black text-cyan-600 dark:text-cyan-400">{formatearDinero(totalHistoricoGeneral)}</p>
                                </div>
                            </Tooltip>
                        </div>
                    )}
            ...
            <div className="animate-fade-in-up delay-150 flex flex-col lg:flex-row w-full gap-4 mb-6">
                ...
                <div className="relative flex-1 group z-10">
                    <svg className="w-5 h-5 absolute left-4 top-3.5 text-gray-400 group-focus-within:text-cyan-500 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    <input
                        type="text"
                        placeholder="Buscar por cliente o RFC..."
                        value={busqueda}
                        onChange={(e) => setBusqueda(e.target.value)}
                        className="w-full pl-12 pr-4 py-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl text-sm font-medium text-gray-900 dark:text-white outline-none focus:border-cyan-500 focus:ring-4 focus:ring-cyan-500/10 transition-all shadow-sm"
                    />
                </div>
                ...
                <div className="relative md:w-64 z-10">
                    <select
                        ...
                        className="w-full pl-4 pr-10 py-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl text-sm font-bold text-gray-700 dark:text-gray-200 outline-none focus:border-cyan-500 transition-all shadow-sm appearance-none cursor-pointer"
                    >
                ...
                <button
                    ...
                    className="bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-3 px-6 rounded-2xl shadow-md transition transform hover:-translate-y-0.5 flex items-center justify-center gap-2 whitespace-nowrap z-10"
                >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg>
                    <span className="hidden sm:inline">Dar de alta cliente</span>
                </button>
            </div>
                ...
                <main className="animate-fade-in-up delay-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    >
                        ...
                        <div className="w-20 h-20 bg-blue-100 dark:bg-blue-900/50 rounded-full flex items-center justify-center mb-4 transition-transform group-hover:scale-110 animate-[pulse_3s_ease-in-out_infinite]">
                            <svg className="w-10 h-10 text-cyan-600 dark:text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4"></path></svg>
                        </div>
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-2 text-center group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">Dar de alta cliente</h3>
                        <p className="text-sm text-gray-500 dark:text-slate-400 text-center px-4">
                            Añade clientes manualmente si no quieres importar sus XMLs.
                        </p>
                    </div>
                    ...
                    {clientesFiltrados.map((cli) => {
                        ...
                        if (estadoCredito === 'vencido') {
                            ...
                            estilosTarjeta = "border-2 border-cyan-400 shadow-[0_0_15px_rgba(34,211,238,0.3)]";
                        ...
                        return (
                            ...
                            >
                                {/* ZONA DE LOGO */}
                                <div className="bg-gray-100 dark:bg-[#e2e8f0] rounded-xl h-28 mt-2 mb-5 flex items-center justify-center p-4 overflow-hidden shadow-inner">
                                    {cli.logo ? (
                                        <img src={cli.logo} alt="Logo" className="max-w-full max-h-full object-contain drop-shadow-sm transition-transform group-hover:scale-105" />
                                    ) : (
                                        <span className={`text-5xl font-black text-transparent bg-clip-text bg-gradient-to-br ${colorFondo} select-none tracking-widest uppercase`}>
                                            {inicial}
                                        </span>
                                    )}
                                </div>
                                ...
                                {/* TÍTULO */}
                                <h3 className="text-gray-800 dark:text-white font-black text-xl leading-tight mb-4 line-clamp-2 transition-colors group-hover:text-cyan-600 dark:group-hover:text-cyan-400" title={cli.nombre}>
                                    {cli.nombre}
                                </h3>
                                ...
                                <div className="flex items-center gap-2 mb-6 flex-wrap">
                                    <Tooltip texto={mensajeSemáforo}>
                                        <span className={`text-xs px-3 py-1.5 rounded-full font-bold flex items-center gap-1.5 cursor-help transition-colors ${estadoCredito === 'vencido' ? 'text-red-700 bg-red-100 dark:text-red-400 dark:bg-red-900/30' :
                                                estadoCredito === 'por_vencer' ? 'text-yellow-800 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-900/30' :
                                                    estadoCredito === 'al_dia' ? 'text-cyan-700 bg-cyan-100 dark:text-cyan-400 dark:bg-cyan-900/30' :
                                            ...
                                            <span className={`w-2 h-2 rounded-full ${estadoCredito === 'vencido' ? 'bg-red-500' :
                                                    estadoCredito === 'por_vencer' ? 'bg-yellow-500' :
                                                        estadoCredito === 'al_dia' ? 'bg-cyan-500' :
                                    ...
                                    <span className="text-xs px-3 py-1.5 rounded-full bg-slate-100 text-slate-600 dark:bg-[#2a3441] dark:text-slate-300 font-bold tracking-wider uppercase border border-slate-200 dark:border-slate-600/50 truncate max-w-[120px]" title={cli.grupo || 'Sin agrupar'}>
                                        📦 {cli.grupo || 'General'}
                                    </span>
                                ...
                                <Tooltip texto="Monto que este cliente te debe." anchoTotal={true}>
                                    <div className={`w-full border rounded-xl p-4 mb-6 bg-gray-50 dark:bg-[#1a1f2b] flex justify-between items-center cursor-help transition-colors ${estadoCredito === 'vencido' ? 'border-red-400/50 dark:border-red-500/30 bg-red-50/50 dark:bg-red-900/10' :
                                            (estadoCredito === 'al_dia' || estadoCredito === 'acumulando') ? 'border-cyan-400/50 dark:border-cyan-500/30 bg-cyan-50/50 dark:bg-cyan-900/10' :
                                        ...
                                        <span className={`text-sm flex items-center gap-1.5 font-medium ${estadoCredito === 'vencido' ? 'text-red-600 dark:text-red-400' :
                                                (estadoCredito === 'al_dia' || estadoCredito === 'acumulando') ? 'text-cyan-600 dark:text-cyan-400' :
                                ...
                                <div className="flex flex-col gap-3 text-sm text-gray-500 dark:text-slate-400 mb-8 font-medium px-1">
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-gray-700 dark:text-slate-200 font-semibold">{cli.metricas?.ultimaCompra ? cli.metricas.ultimaCompra.split('T')[0] : 'Sin registro'}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span>Monto Histórico:</span>
                                        <span className="text-gray-700 dark:text-slate-200 font-semibold">{formatearDinero(cli.metricas?.comprasHistoricas)}</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-4 mt-auto">
                                    <button
                                        ...
                                        className={`flex-1 text-sm font-bold py-3.5 rounded-xl transition-colors shadow-sm ${estadoCredito === 'vencido' ? 'bg-red-100 hover:bg-red-600 dark:bg-red-900/40 dark:hover:bg-red-600 text-red-700 hover:text-white dark:text-red-400 dark:hover:text-white' :
                                                (estadoCredito === 'al_dia' || estadoCredito === 'acumulando') ? 'bg-cyan-100 hover:bg-cyan-600 dark:bg-cyan-900/40 dark:hover:bg-cyan-600 text-cyan-700 hover:text-white dark:text-cyan-400 dark:hover:text-white' :
                                                    'bg-gray-100 hover:bg-cyan-600 dark:bg-[#2a3441] dark:hover:bg-[#323d4d] text-gray-700 hover:text-white dark:text-slate-300 dark:hover:text-white'
                                            }`}
                                    >
